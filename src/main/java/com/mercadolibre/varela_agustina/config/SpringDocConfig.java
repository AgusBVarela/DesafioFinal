...
import static com.mercadolibre.varela_agustina.util.ScopeUtils.SCOPE_VALUE;
import static com.mercadolibre.varela_agustina.util.ScopeUtils.isLocalScope;
...
public class SpringDocConfig {
	...
	public OpenAPI customOpenAPI(@Value("${app.title}") String appName, @Value("${app.description}") String description, @Value("${app.version}") String version) {
		OpenAPI api = new OpenAPI().info(new Info().title(appName)
				...
				.contact(new Contact().name("pigino-sofia")
						.email("sofia.pigino@mercadolibre.com")));
